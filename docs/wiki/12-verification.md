...
冻结 24 只 A 股两年行情、使用 2026-03-12 至 2026-07-10 共 82 个交易日留出段时，20 日调仓的研究版行为因子收益 `+63.8573%`、换手 `254.6893%`、最大回撤 `-11.0715%`；当时委员会确定性基座分别为 `-12.9719%`、`5449.2038%`、`-21.6536%`。成对 5 日区块 bootstrap 的收益差异 95% 区间为 `[39.7654, 125.7515]` 个百分点。

该结果支持“行为因子比当时高换手基座更适合作为候选生产基座”，但不证明当前生产配置未来盈利。研究胜出版本是 20 日调仓；当前生产按用户要求改为 5 日目标更新，并使用每标的最近三个月收益调整优化器权重。生产配置必须另行做完整留出段和成本后验证。公式与调用链见 [16-A 股行为因子](16-ashare-behavioral-factor.md)。
