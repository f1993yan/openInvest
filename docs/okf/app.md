...
- **Factor Target Overview**: `MainScreen.kt` reads optional `HoldingRow.behavioral_factor` values from the current snapshot, renders up to four selected model targets, and opens the existing committee dialog on tap. It never writes the ledger or treats model selection as account ownership.
