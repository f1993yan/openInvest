...
def build_config(args: argparse.Namespace) -> LaunchConfig:
    ...
    return LaunchConfig(
        ...
        port=env_int("INVEST_BACKEND_PORT", args.port),
...
def ensure_layout(config: LaunchConfig) -> None:
    ...
    if shutil.which(config.uv) is None and not Path(config.uv).exists():
        raise SystemExit(
...
def launch(config: LaunchConfig) -> None:
    ...
    popen_hidden(
        config,
        "market monitor",
        ["run", "python", "-m", "jobs.market_monitor"],
    ...
    if wait_for_port(config.host, config.port):
        print(f"Backend ready: http://{config.host}:{config.port}")
    else:
        print(
            f"Backend process started but port {config.port} was not ready yet. "
            f"Check {config.log_dir / 'backend.err.log'} if it does not recover."
        )
        if backend.poll() is not None:
            print(f"Backend exited early with code {backend.returncode}.", file=sys.stderr)
