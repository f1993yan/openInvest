...
6. **A股交易规则可行性（{{asset_symbol}} 以 .SS/.SZ 结尾时必查）**:
   - **最小买入单位 = 100 股（1 手），且只能按 100 股整数倍加仓**。所以"一手成本" = 现价 × 100。
     如果 INVESTABLE_CASH_CNY < 现价 × 100，**这次加仓在物理上不可行**——必须直接给
     SIGNAL=concerned 并在 ONE_LINER 写"子弹不足 1 手（需 ¥X），无法加仓"，不要建议买零股。
   - 加仓金额建议必须能换算成整手：把"建议建仓比例上限"折算成 **可买手数**
     （floor(可投金额 / (现价×100)) 手），而不是一个买不出来的零碎金额。
   - **T+1 交易**：A股当天买入次日才能卖出。这意味着加仓后至少锁定 1 个交易日，
     无法日内止损。评估止损/压力测试时要计入"买入后 T+1 内无法离场"的隔夜跳空风险，
     高波动（atr_pct 高）标的尤其要留更厚的安全垫，建仓比例相应更保守。
...
**A股整手 + T+1 硬约束（{{asset_symbol}} 以 .SS/.SZ 结尾时强制）**：
- 最小买入 = 100 股整手，加仓只能是 100 股整数倍。**一手成本 = 现价 × 100**。
- INVESTABLE_CASH_CNY < 现价 × 100 → 加仓不可行，SIGNAL=concerned，ONE_LINER 写
  "子弹不足 1 手（需 ¥现价×100），无法加仓"，**禁止**建议买零股或给买不出的零碎金额。
- 给加仓建议时换算成 **可买手数** = floor(可投上限 / (现价×100))，金额按整手回报。
- T+1：当天买入次日才可卖，加仓后至少锁仓 1 个交易日、无法日内止损。压力测试与止损
  评估必须计入"买入后 T+1 内无法离场"的隔夜跳空风险；atr_pct 越高，安全垫留越厚、
  建仓比例越保守。
（港股 .HK 每手股数不固定，见 brief；美股/加密无整手约束，不适用本条。）
