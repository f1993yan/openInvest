...
# 龙头股阈值：持仓 ≥ 此百分比的才进周日晚 summary + 委员会
...
_SUMMARIZE_PROMPT = """你是一个资深 A 股/港股宏观分析师。下面是从周六到周日从多个国内财经新闻源
（东方财富、财联社、新浪财经、雪球、华尔街见闻、百度热搜）抓取的所有新闻标题和摘要。

请完成以下分析：

## 1. 周末关键主题（3-5个）
用一两句中文概括本周末最重要的宏观/板块/个股主题。

## 2. 龙头股影响评估
对于下面列出的每一只龙头股，评估周末新闻对它的影响：
- **正面** (+1): 有明确利好
- **中性** (0): 无明显关联或多空交织
- **负面** (-1): 有明确利空
...
{{
  ...
  "stock_impact": {{
    "SYMBOL": {{"impact": 1, "reason": "理由"}},
    ...
  }},
  "need_committee_rerun": ["SYMBOL1", "SYMBOL2", ...],
...
def summarize_and_evaluate(caches: List[Dict[str, Any]],
    ...
    """LLM 总结周末新闻 + 评估龙头股影响"""
    if not caches:
        return {"error": "无周末缓存数据", "need_committee_rerun": []}

    total_items = sum(c.get("total_items", 0) for c in caches)
    log.info(f"开始 LLM 总结: {len(caches)} 个缓存, 共 {total_items} 条新闻")

    news_text = _build_news_text(caches)
    leading_text = _build_leading_stocks_text(config)

    holdings = config.get("holdings", [])
    leading_stocks = [
        h for h in holdings
        if h.get("position_pct", 0) >= LEADING_PCT_THRESHOLD
    ]
    leading_symbols = [h["symbol"] for h in leading_stocks]

    prompt = _SUMMARIZE_PROMPT.format(
        news_text=news_text[:12000],  # 截断防 token 超限
        leading_stocks=leading_text,
    )
    ...
    if not api_key:
        return {"error": "LLM API key 未设，无法总结", "need_committee_rerun": []}
    ...
    try:
        ...
        return {"error": f"LLM 调用失败: {e}", "need_committee_rerun": []}
    ...
    # 确保 need_committee_rerun 只包含实际持仓的 symbol
    valid_symbols = set(h["symbol"] for h in holdings)
    rerun = [
        s for s in result.get("need_committee_rerun", [])
        if s in valid_symbols
    ]

    # 如果 LLM 没有推荐任何标的，默认把所有龙头股都送给委员会
    if not rerun and leading_symbols:
        log.info("LLM 未推荐具体标的，默认评估所有龙头股")
        rerun = leading_symbols

    result["need_committee_rerun"] = rerun
    result["_total_news"] = total_items
    result["_cache_count"] = len(caches)
    result["_leading_stocks"] = leading_symbols
...
def run() -> Dict[str, Any]:
    ...
    if not is_sunday_evening and not force_eval:
        ...
        return {
            ...
            "next_step": "等待周日晚 20:00 进行总结 + 委员会评估",
        }

    log.info("周日晚间模式：加载周末缓存 → 总结 → 委员会评估")
    ...
    if not config.get("holdings"):
        log.error("无持仓配置，跳过委员会")
        return {"status": "error", "error": "market_monitor_config.json 缺失或 hold holdings"}
    ...
    log.info(f"总结完成: sentiment={summary.get('overall_sentiment')}, "
             ...
             f"need_rerun={summary.get('need_committee_rerun', [])}")
    ...
    report = {
        ...
        "summary": {
            ...
            "stock_impact": summary.get("stock_impact", {}),
        },
        "committee": {
