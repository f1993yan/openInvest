...
- 前四成员与目标权重每 5 个交易日更新，保存在 git 忽略的 `data/behavioral_factor_state.json`；当前仓位距离目标不超过 5 个百分点时不因因子重复交易，但可靠止盈止损卖出不受此限制。
...
- 已有持仓触发止损/止盈线时，提醒层还会做板块恐慌杀跌保护：用同板块监控样本的中位跌幅、下跌占比、硬跌幅占比、相对全样本中位数的超额跌幅和个股相对板块 MAD Z 值判断是否属于板块共振下跌。如果个股没有明显弱于板块，`sector_panic_guard` 会把可能错杀/错过反弹的成本计入继续持有证据，要求卖出净效用明显为正才进入 `需要操作`；如果个股显著弱于板块或委员会强卖出压力很高，则不拦截卖出。
