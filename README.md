...
- 交易模式：桌面窗口可切换主动盈利、现金回收，改变提醒优化层的现金保留和买卖执行门槛。
...
可选值：`active_profit`（主动盈利，默认）、`cash_recovery`（现金回收，保留更多现金并优先释放风险/现金）。模式只作用于 `jobs/market_monitor_alerts.py` 的提醒优化层，不改变委员会原始结论字段。历史配置中的 `risk_off` 会自动按 `cash_recovery` 处理，不再作为独立模式提供。
...
3. 确定性优化器根据现金、手数、regime、概率、基本面和风控约束修正仓位；A 股有足够历史与横截面时，以行为因子替换旧技术回退基座。
...
生产监控和日度选股共用 `core/ashare_behavioral_factor.py`：

- 因子分数 = `65% 多周期动量 + 25% 低换手代理 + 10% 低波动`。多周期动量内部使用 60/120/252/20 日收益的横截面排名。
- 资格门要求 60 日收益为正且收盘价位于 EMA100 上方；合格标的取前四，按 20 日波动率倒数分配，组合目标投入最高 95%，单股最高 35%。
- 日度选股先由国内新闻、板块资金流和北向资金生成候选池，再由行为因子执行资格门和前四硬过滤；基本面、资金、风险和技术时点只能排列、解释这四只，不能把未进入因子前四的候选重新加入结果。
- 每只标的的优化器权重独立计算：回看最近 63 个交易日，按 5 日持有块复盘该标的因子方向相对样本池的超额收益，通过经验贝叶斯可靠度和后验正收益概率得到 `0.55~1.0` 权重。
- 前四成员与目标权重每 5 个交易日更新，保存在 git 忽略的 `data/behavioral_factor_state.json`；当前仓位距离目标不超过 5 个百分点时不因因子重复交易，但委员会/优化器给出的高置信卖出仍可越过免交易带。
- 桌面主窗口和 Android 主界面从监控快照读取 `selected=true` 的标的，按 `target_weight_pct` 降序列出最多四只；点击目标可定位卡片或打开委员会分析。该列表是因子模型目标组合，不是实际持仓，真实持仓仍只读取 `AccountLedger(real)`。
- 因子只替换 A 股确定性基座。港股等非 A 股、真实/影子账户隔离、A 股 T+1、整数手、科创板首次 200 股、涨跌停和现金约束保持原逻辑。
- `behavioral_factor` 是委员会请求/响应的可选字段，旧 App 可以忽略。直接单标的调用无法形成有效横截面时会明确标记低置信度并回退，不会伪造前四结论。
- 周日任务内部保留持仓与关注列表的完整横截面供委员会计算零目标，但面向用户的 `data/behavioral_factor_assessments.json` 只列前四，并记录候选数和入选数。

完整数学口径、字段和验证边界见 [A 股行为因子](docs/wiki/16-ashare-behavioral-factor.md)。

### A 股卖出提醒

生产链路已停用成本锚定的持仓纪律影响。`position_exit_plan`、`position_exit_policy` 和 `position_exit_discipline_review` 不再影响优化器预期收益、委员会审核、提醒阈值、主窗口状态或详情文案。

- `entry_exit_points` 仍用于回调、突破、重新入场、技术止损和技术止盈参考。
- `SELL/TRIM` 是否进入 `需要操作` 只由委员会/确定性优化器方向、负向建议金额、卖出评分、交易模式、可卖手数、A 股 T+1 和重复交易护栏决定。
- 已持仓 A 股不会因为旧 `position_exit_plan` 触发止损/止盈而生成卖出提醒；旧快照里的 `position_exit_discipline_review` 会被快照层忽略。
- 后端直连和 Android Chaquopy 本地委员会执行相同口径，仍返回空的 `position_exit_policy` 兼容对象，旧客户端无需修改字段解析。
- 当天或冷却窗口内发生过同标的真实/影子反向成交时，再次买入必须重新触发回踩、再入场或突破线，并且价格相对上次成交价已经穿越对应边界；再次卖出不再要求持仓纪律线，只要求委员会/优化器卖出方向和交易约束成立。
- 成本锚定策略模块和周度调参任务已删除，`.env.example` 不再提供对应参数；历史回测脚本只允许离线比较，不再写回生产 `.env`。
- A 股缺少有效行为因子时，后端会用目标标的、真实持仓和关注列表补建横截面；仍无法补全时返回“无法判断”，桌面显示灰色卡片并禁用交易按钮，不会回退旧技术收益模型。
- 日度选股同样不再使用无行为因子的旧新闻/技术加权兜底；无法形成可靠横截面的候选会被过滤，并在结果摘要记录 `factor_unavailable_filtered`。
- 盘中监控启动后仍会读取 `data/sector_cache.json`，对 A 股标的用东方财富板块覆盖运行时 `sector`，并保留原配置为 `config_sector`。这只影响委员会上下文、告警板块分布和窗口展示，不再用于持仓纪律参数读取。
- 如需盘中出现 `需要操作` 时发邮件，配置 `EMAIL_SENDER`、`EMAIL_PASSWORD`、`DIGEST_EMAIL_TO` 并设置 `.env` 的 `INVEST_MONITOR_EMAIL_ACTIONS=1`。这是邮件总开关；桌面窗口标题栏的 `邮件开/邮件关` 会写入 `jobs/market_monitor_config.json` 的 `monitor_action_email_enabled`，可在运行中临时关闭执行邮件。邮件会列出标的、买/卖方向、推荐手数、LLM 审核手数差异、触发来源和原因；同一交易日相同触发只发送一次。

历史卖出参数诊断脚本仍可用于研究，但不代表当前生产提醒逻辑：
...
诊断报告写入 `reports/ashare_sell_threshold_diagnosis.json`，重点看 `decision_utility`、`max_drawdown_pct`、`sell_win_rate_lower` 和 `avg_net_sell_path_edge_pct`。这些字段只用于解释历史止盈止损参数质量，不会改变当前监控窗口的买卖提醒。
...
- “因子目标前四”区域按模型目标仓位排序，桌面端点击名称切换到对应卡片；入选标的卡片显示目标仓位，但不会因此显示成真实持仓或修改账本。
- `需要操作`、`触发确认`、`单轮触发` 优先于普通观察。
- `候选卖/候选买` 表示委员会有方向，但尚未满足提醒层综合阈值、现金/仓位约束或交易护栏。
...
- 周末新闻通过主窗口按钮打开，以相同风格的悬浮卡片展示并按影响力排序；标题区同时显示新闻来源日期和摘要生成文件，避免把补跑日期误认为新闻日期。每只龙头标的可直接点击进入委员会分析，未关注标的会先解析代码和最新行情。
...
- **周末新闻机会标的互动**：在“周末新闻与机会”中点击龙头股名称会直接关闭新闻页并打开对应委员会分析；已监控标的复用当前快照，未关注标的进入解析进度状态后自动补全代码和行情。
- **移动端本地 AI 投委会辩论**：从新闻龙头、监控卡片或其他标的入口打开分析时，对未追踪的股票会自动通过网络接口解析 symbol，并在手机本地的 Chaquopy Python 环境下调用 LLM（Gemini/DeepSeek）异步执行多角色辩论与优化仓位策略。
- **后台悬浮信号岛**：自动刷新服务在 App 退到后台后可显示系统悬浮窗，分析中展示进度，出现买卖信号时以紧凑卡片提醒；需要 Android 悬浮窗权限，可在设置中关闭。
- **交易模式同步**：App 设置中的 `主动盈利 / 现金回收` 会传入手机本地 `run_committee_local`，影响委员会提示语和提醒优化口径；旧版调用不传该参数时默认 `active_profit`。
- **行为因子可见**：主界面单独列出按目标仓位排序的行为因子目标前四，点击后打开对应标的委员会分析；入选卡片和委员会详情展示目标仓位、因子分数、近三个月收益/命中率/样本数和标的独立优化器权重。该区域固定说明“模型目标，不是持仓”。
...
- 板块大资金流向；
- 板块内部个股的基本面和技术面；
- A 股行为因子资格门、横截面分数和目标前四成员；
- 1/5/20 日收益路径分布、ATR 风险、防守惩罚和历史相似样本命中率；
- 账户现金约束，过滤资金不足以买入一手的标的。

结果写入 `data/daily_stock_selection/latest.json`，其中 `stocks`、`reference_pool` 和 `action_pool` 只包含 `behavioral_selected=true` 的成员，最多四只。现金不足、历史不足或技术数据不可用时可以少于四只，不会用行为因子第五名递补。桌面窗口底部只展示这份最终名单；点击按钮会打开悬浮详情，展示为什么入选、什么位置更适合买入、风险在哪里，以及是否值得加入关注列表。

如果 `latest.json` 里板块或新闻有数据、但 `stocks` 为空，先查看摘要中的 `factor_unavailable_filtered`、`factor_not_selected_filtered` 和 `affordability_filtered`。日度选股需要至少 252 根有效日线形成可靠因子；候选全部未进前四或无法买入一手时，空结果是预期行为。当前 A 股历史日线优先使用东方财富/腾讯直连 HTTP。
