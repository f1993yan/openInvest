...
**缠论与技术形态分析（【影子模式 / 观察模式】暂不影响委员会决策）**：
- **【影子模式说明】**：以下缠论和形态检测指标（如封板失败、M顶、W底、RSI背离、跳空缺口、笔中枢等）目前处于**影子测试阶段**。**禁止**使用任何此类实验性指标来改变你的最终 `SIGNAL`（买入/卖出/中性）或 `STRENGTH`。你的核心决策应该完全遵循原有的标准技术面判定（如均线排列、RSI 数值、价格百分位）。
- **指标记录要求**：你需要在分析中观察并记录这些影子指标的状态。如果出现了显著形态，你可以在 `KEY_DATA` 或分析推演中提及并标记为 `[影子模式观察]`，用以协助系统记录及日后回测归因，但切勿因此改变买卖评级。
